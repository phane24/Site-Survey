...
<head>
	...
	<script>
		...
		{			
			$.ajax({
               type:"get",
               url:"getClosedSurveySites",
               contentType: 'application/json',
               datatype : "json",
               success:function(data) {
                   closedSurveyList = JSON.parse(data);
					
                   for(var i=0;i<closedSurveyList.length;i++)
        		   {
					
                   	dataSet.push([closedSurveyList[i].siteid,closedSurveyList[i].surveyStatus,closedSurveyList[i].surveyFilename]);
  
        		   }
                  
                   
			 var table1=$('#closedSitesTable').DataTable({
					destroy:true,
					language: {
					...
					columnDefs: [
						...
						{ "targets": -1, "data": null, "defaultContent": "<button style=' background-color: #4CAF50;border: none;  color: white;  padding: 5px 25px;  text-align: center;  text-decoration: none;  display: inline-block;  font-size: 16px;  margin: 4px 2px;  cursor: pointer;' id='viewBtn'>Download</button>"}],
			        data: dataSet,
			        columns: [
						{title: "Site Id" },
						{title: "Site Status" },
...
// 						{title: "Manager Name" },
			 ...
			 $('#closedSitesTable tbody').on('click', '[id*=viewBtn]', function () {
...
// 						$("#imageId").css("display","block");
				         ...
				         window.location.href = "/sitesurvey/downloadFile?pdfFileName="+window.encodeURIComponent(pdfFile);
...
function getSurveyCount(){
			...
			$.ajax({
		        type:"get",
		        url:"surveySitesCount",
		        contentType: 'application/json',
		        datatype : "json",
		        success:function(result) {
		        	var jsonArr = $.parseJSON(result);
		        	 $('#assignedSurvey')[0].innerHTML=jsonArr.AssignedSurvey;
			         	$('#unassignedSurvey')[0].innerHTML=jsonArr.UnAssignedSurvey;
		               $('#closedSurvey')[0].innerHTML=jsonArr.ClosedSurvey;
				...
				<div class="page-inner">
					...
					<div class="row">
						<div class="col-sm-6 col-md-3">
							...
							<div class="card-body" onclick="location.href='${pageContext.request.contextPath}/assignedSurvey'"	style="cursor: pointer;" >
						...
						<div class="col-sm-6 col-md-3">
							<div class="card card-stats card-round">
								<div class="card-body" onclick="location.href='${pageContext.request.contextPath}/closedSurvey'" style="border-radius: 10px;background-color:#00B1BF;cursor:pointer;">
									<div class="row align-items-center">
										<div class="col-icon">
											<div class="icon-big text-center bubble-shadow-small" style="background:#808080;border-radius: 5px;">
											<img src="<c:url value='resources/assets/img/history.svg' />" >
										...
										<div class="col col-stats ml-3 ml-sm-0">
											<div class="numbers">
												...
												<h4 class="card-title" id="closedSurvey" style="color:#ffffff;"></h4>
											</div>
										</div>
									</div>
								</div>
					...
					<div class="row">
							...
							<div class="card">
								<div class="card-header">
									<h4 class="card-title">Sites Survey</h4>
								</div>
								<div class="card-body">
									<div class="table-responsive">
										<table id="closedSitesTable" style="width:100%" class="display table table-striped table-hover" ></table>
									</div>
								</div>
							</div>
						</div>
